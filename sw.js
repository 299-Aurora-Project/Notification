...
<title>Aurora Forecast Pro v. t1.4.wip1</title>
...
<div id="home-screen" class="container">
    <div class="header-area">
        <div class="title-row">
            <h1>🌌 Aurora Forecast Pro<span class="beta-suffix">(beta)</span> <span class="version">v. t1.4.wip1</span></h1>
        </div>
...
// GASの /exec で終わるURL
const GAS_URL = 'https://script.google.com/macros/s/AKfycbzPPBpwQwJZfx0Yc88KKhzUO5e4xMJEATYf8SP98qtytvC6yg2n84nxJH4W9PLYGXNh/exec'; 
// ----------------------
...
async function testNotification() {
    ...
    if (permission === "granted") {
        const isSaved = await subscribeUser();
        if (isSaved) {
            // 2. 保存に成功したらGASに「自分へのテスト通知」を依頼する
            await fetch(GAS_URL + "?action=test", { mode: 'no-cors' });
            alert("Sent test request to GAS server!");
        } else {
            alert("Failed to save registration to Spreadsheet.");
        }
    } else {
...
async function updateAll() {
    // 修正：ロケーション未設定時の処理を追加
    if (myLat === null) {
        // ウェルカムオーバーレイを再表示してユーザーに通知する
        document.getElementById('welcome-overlay').style.display = 'flex';
        // あるいはシンプルにアラートを出す場合: alert("Please set a location first");
        return;
    }
...
function calculateTotal() {
    ...
    if (displayScore >= 70) {
        ...
        resultEl.innerHTML = `Low${(potentialScore >= 35 && evalCloud > 70 ? '<span class="cloud-icon">☁️</span>' : '')}`;
...
function initMap() {
    ...
    // --- 追加箇所：マーカーを最前面に持ってくるための設定 ---
    map.createPane('userMarkerPane');
    map.getPane('userMarkerPane').style.zIndex = 650; // オーロラレイヤー(400番台)より高い数値
    map.getPane('userMarkerPane').style.pointerEvents = 'none'; // 地図操作の邪魔をしない
    // ------------------------------------------------
...
function updateMapMarker() {
    ...
    userMarker = L.circleMarker(pos, { 
        ...
        radius: 8, 
        stroke: false,        // weight: 0 --> 枠線を完全にオフ
        fillOpacity: 1, // 不透明度1.0でオーロラが透けないようにする
        pane: 'userMarkerPane' // 最前面レイヤーを指定
    }).addTo(map);
}
...
async function shortForecast() { try { const res = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${myLat}&longitude=${myLon}&hourly=cloudcover`); const data = await res.json(); cloudDataHourly = data.hourly.cloudcover; const hr = new Date().getHours(); let html = (hr >= 21 || hr < 3) ? `<div>🕒 In 3h: Clouds ${formatProbability(cloudDataHourly[3])}</div>` : `<div>🌙 Tonight(21:00): Clouds ${formatProbability(cloudDataHourly[21])}</div>`; html += `<div>📅 Tomorrow(21:00): Clouds ${formatProbability(cloudDataHourly[45])}</div>`; document.getElementById("forecast").innerHTML = html; } catch(e) { } }
function setItemLoading(id, isLoading) { const el = document.getElementById(id); if (el && isLoading) el.innerHTML = '<span class="loading-text">⌛ <i>Loading...</i></span>'; }
...
async function searchCity() {
    ...
    try {
        ...
        if (data.results?.length > 0) {
            ...
            // 画面のズーム/位置をリセット
            //window.scrollTo({ top: 0, behavior: 'smooth' }); // 面最上部へスムーズに戻る
...
function closeAllLists() {
    clearTimeout(searchTimer); // ここでもタイマーを止める
    const list = document.getElementById("autocomplete-list");
    if (list) {
        list.innerHTML = "";
    }
...
/* Notificationスイッチ */
// 「Notification」スイッチが切り替わった時に、現在の緯度・経度とスイッチの状態をセットでGASに送信するようにします。
// asyncを付けて非同期関数にします
async function handleNotifToggle(checkbox) {
    ...
    // --- 2. 裏側でGASにデータを送信（ユーザーを待たせない） ---
    // 通信に失敗してもスイッチが戻らないよう、try-catchで囲みます
    try {
        const registration = await navigator.serviceWorker.ready;
        ...
        if (sub) {
            ...
            // await fetch を使うが、この後にはUI処理がないので描画には響かない
            await fetch(GAS_URL, {
                method: 'POST',
        ...
        console.error("GASへの同期に失敗しました:", e);
        // ここで alert を出すと描画が止まるので、console.log に留めるのがスムーズです
    }

    // --- 3. 最後にアラート（以前の15ms遅延を継承） ---
    setTimeout(() => {
        const msg = isEnabled ? 
            `Notifications have been set.\nTarget area: ${document.getElementById("loc").textContent}` : 
            `Notifications have been cancelled.`;
        alert(msg);
    }, 50); // 通信時間を考慮して少しだけ長めに設定
}
